...
TPolynomial::TPolynomial(const TPolynomial &A)
{
  ...
  else
  {
    ...
    for ( TMonomial* it = A.start->GetNext(); it != nullptr; it=it->GetNext() )
    {
...
TPolynomial &TPolynomial::operator-=( const TMonomial &m)
{
  ...
  {
    TMonomial* last_el = nullptr;
    for ( TMonomial *ptr = start; ptr != nullptr; ptr=ptr->GetNext() )
    {
      if ( ptr->ComparePowers(m) )
      {
        *ptr -= m;
...
TPolynomial TPolynomial::operator*(TPolynomial &A)
{
  ...
  for ( TMonomial* src_ptr = start; src_ptr != nullptr; src_ptr = src_ptr -> GetNext() )
    for ( TMonomial* p_ptr = start; p_ptr != nullptr; p_ptr = p_ptr -> GetNext() )
    {
      tmp += (*src_ptr) * (*p_ptr);
    }
  return tmp;
